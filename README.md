...
*   **レスポンシブデザイン**: スマートフォンでも読みやすいダイナミックなUI（タイトルサイズの自動調整など）

## 🔒 セキュリティとプライバシーについて (Security Note)
*   **ファイル公開範囲**: システムにアップロードされたファイルは、URLを知り得る人であればアクセス可能な状態（Public Access）になりますが、URLには予測困難なランダム文字列が含まれます。
*   **削除ロジック**: アプリ上でドキュメントを削除すると、紐づくクラウド上のファイルも自動的に削除され、以降はアクセス不可（404）となります。
*   **認証**: アプリケーションの閲覧にはログインが必須です。外部の未認証ユーザーはコンテンツを閲覧できません。
